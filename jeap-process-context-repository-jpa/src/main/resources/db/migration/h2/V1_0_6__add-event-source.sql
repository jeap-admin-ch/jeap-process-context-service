...
SELECT RANDOM_UUID() as id, event_id, event_name, NULL as idempotence_id, MIN(created_at) as created_at
FROM event
...
EXECUTE IMMEDIATE 'ALTER TABLE process_update DROP CONSTRAINT ' ||
                  QUOTE_IDENT((SELECT CONSTRAINT_NAME
                               FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
                               WHERE TABLE_NAME = 'process_update' AND CONSTRAINT_TYPE = 'UNIQUE'));
...
INSERT INTO events (id, event_id, event_name, idempotence_id, created_at)
SELECT RANDOM_UUID() as id, event_id, event_name, idempotence_id, created_at as created_at
FROM process_update
where handled = false
...
SELECT RANDOM_UUID() as id, es.id as events_id, p.id as process_instance_id, CURRENT_TIMESTAMP as created_at
FROM process_instance p
INNER JOIN event e ON e.process_instance_id = p.id
INNER JOIN events es ON es.event_id = e.event_id AND es.event_name = e.event_name
group by p.id, es.id;
