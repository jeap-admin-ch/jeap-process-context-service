...
### Dependencies
- **ch.admin.bit.jeap:jeap-spring-boot-parent**: 41.7.0 → 41.9.0 (minor)
...
- **systeminformation**: 5.33.10 → 5.33.11 (patch)
- **prettier**: 3.9.6 → 3.9.7 (patch)
- **ng-mocks**: 14.17.6 → 14.18.0 (minor)
...
- **org.codehaus.mojo:build-helper-maven-plugin**: 3.6.1 → 3.6.2 (patch)
- **com.microsoft.playwright:playwright**: 1.62.0 → 1.63.0 (minor)
- **ng-mocks**: 14.17.5 → 14.17.6 (patch)
- **fast-uri**: 4.1.4 → 4.1.5 (patch)
...
- **systeminformation**: 5.33.5 → 5.33.8 (patch)
- **qs**: 6.15.3 → 6.16.0 (minor)
- **jest-environment-jsdom**: 30.5.0 → 30.5.1 (patch)
- **jest**: 30.5.0 → 30.5.1 (patch)
...
### Changed
- Hold `@cypress/vite-dev-server` on 7.x in Renovate: 8.0.0 requires Vite 8 and Cypress 16, which only become
  available with the Angular 22.1 stack.

## [27.1.3] - 2026-08-28

### Added
- Document PCS maintenance jobs with canonical request, CSV, report, and OpenAPI examples.
...
- **systeminformation**: 5.33.1 → 5.33.4 (patch)
- **picomatch**: 4.0.5 → 4.0.7 (patch)
- **ng-mocks**: 14.17.2 → 14.17.3 (patch)
- **fast-uri**: 4.1.2 → 4.1.3 (patch)
- **eslint**: 10.8.1 → 10.9.1 (minor)
- **cypress**: 15.21.0 → 15.21.1 (patch)
- **@typescript-eslint/parser**: 8.67.0 → 8.68.0 (minor)
- **@typescript-eslint/eslint-plugin**: 8.67.0 → 8.68.0 (minor)
- **@analogjs/vite-plugin-angular**: 2.7.0 → 2.7.1 (patch)

## [27.0.2] - 2026-08-25

### Fixed
- Resolve static analysis findings in relation-republication tests.

## [27.0.1] - 2026-08-24

### Added
- Add durable jobs for republishing explicitly selected persisted relations through the existing relation listener.
...
### Added
- Dispatch relation-reevaluation tasks through the transactional outbox and execute them idempotently against all
  stored process data.
...
### Changed
- Replaced the frontend license checker with `@jeap/jeap-frontend-license-checker`. The generated third-party notices now
  carry the full license texts of the redistributed dependencies, and the license steps run as part of the npm frontend
  build.
...
- Update **ch.admin.bit.jeap:jeap-spring-boot-parent** from 38.3.0 to 39.0.0, resolving CVE-2026-54428 by upgrading
  **org.apache.httpcomponents.core5:httpcore5-h2** from 5.4.2 to 5.4.3.
...
- Update Angular to 21.2.19 and fast-uri to 3.1.5 to fix security vulnerabilities.
...
- Documentation in `docs/`: getting started, architecture, process templates, configuration, user interface,
  operations and template migration.

### Changed
- The frontend build is no longer bound to a profile, it always runs.

### Fixed
- Add the missing `processRelationPatterns` definition to the process template JSON schema. Templates using
  process relations were flagged as invalid by the schema although they are supported.
...
### Fixed
- Update transitive frontend dependency fast-uri 3.1.2 → 3.1.4 to fix CVE-2026-13676 and CVE-2026-16221
...
### Added
- Implement ui browser integration tests.
...
- Add missing `test` scope to test/mock dependencies: `spring-boot-starter-test` in `jeap-process-context-plugin-api`, `jeap-process-context-domain-test` in `jeap-process-context-repository-jpa` and `jeap-process-context-adapter-rest-api`, and `spring-security-test` in `jeap-process-context-adapter-rest-api`

## [21.0.0] - 2026-06-30

### Dependencies
- **ch.admin.bit.jeap:jeap-spring-boot-parent**: 35.11.0 → 36.1.0 (major)
- **org.codehaus.mojo:exec-maven-plugin**: 3.5.0 → 3.6.3 (minor)
- **org.codehaus.mojo:build-helper-maven-plugin**: 3.6.0 → 3.6.1 (patch)
...
- Update frontend dependencies to fix security vulnerabilities (Angular to 21.2.17, form-data, qs)

## [20.4.0] - 2026-06-12

### Changed

- Update parent from 35.5.0 to 35.6.0
...
- Update js-cookie to 3.0.8 due to CVE-2026-46625
...
- Update quadrel-enterprise-ui/auth to fix Lodash vulnerability
...
- Updated @quadrel-enterprise-ui/auth to version 20.1.2
...
- Refactor app.module.ts to inject QdConfigService directly and remove deferred injector usage
...
- Update parent from 31.2.0 to 31.4.0

## [17.1.1] - 2026-03-18

### Changed

- Add property `jeap.processcontext.kafka.topic-check` to disable topic check at startup in profiles other than local
- Update parent from 31.0.0 to 31.2.0
...
- Add configuration property "jeap.processcontext.kafka.message-consumer-paused" (default false) to be able to pause
  processing of messages in preparation of next major version
- Remove property jeap.processcontext.process-updates.auto-start

## [16.2.1] - 2026-01-28

### Changed

- Fixed SVG icon set by using icons from assets/obliqueIcons.svg.
...
- Update parent from 30.12.0 to 30.14.0

## [16.0.0] - 2026-01-23

### Removed

- The milestone feature has been removed. Milestones are no longer supported in process templates and
  the milestone data of existing process instances is deleted during the database migration. Milestones have not been
  used in any PCS instances.

## [15.0.0] - 2026-01-21

### Removed

- Process instance creation using the REST API has been removed, process instances are created using domain events
  or CreateProcessInstanceCommands only.
...
- Update parent from 30.0.1 to 30.1.0

## [14.5.0] - 2025-12-03

### Changed

- Performance optimization: Avoid correlation queries if process templates do not use correlation by process data
...
- Update to Oblique 14 and Angular 20
